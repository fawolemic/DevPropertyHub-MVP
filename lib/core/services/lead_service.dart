...
class LeadService {
  ...
  /// Get leads by status
  Future<List<LeadModel>> getLeadsByStatus(String developerId, LeadStatus status) async {
    try {
      final response = await _client
          .from('leads')
          .select()
  ...
  Future<List<LeadModel>> getLeadsByPriority(String developerId, LeadPriority priority) async {
    try {
      final response = await _client
          .from('leads')
          .select()
