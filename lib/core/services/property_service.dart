...
class PropertyService {
  ...
  Future<List<PropertyModel>> getPropertiesByDeveloperId(String developerId) async {
    try {
      final response = await _client
          .from('properties')
          .select()
      ...
      return response.map((property) => PropertyModel.fromMap(property)).toList();
    } catch (e) {
  ...
  Future<List<PropertyModel>> getPropertiesByDevelopmentId(String developmentId) async {
    try {
      final response = await _client
          .from('properties')
          .select()
  ...
  Future<List<PropertyModel>> getPropertiesByStatus(String developerId, PropertyStatus status) async {
    try {
      final response = await _client
          .from('properties')
          .select()
  ...
  Future<List<PropertyModel>> getPropertiesByType(String developerId, PropertyType type) async {
    try {
      final response = await _client
          .from('properties')
          .select()
  ...
  Future<List<PropertyModel>> searchProperties(String developerId, String query) async {
    try {
      final response = await _client
          .from('properties')
          .select()
  ...
  Future<List<String>> uploadPropertyImages(String propertyId, List<dynamic> images) async {
    try {
      final imageUrls = <String>[];
      
      for (int i = 0; i < images.length; i++) {
        final file = images[i];
        final fileName = '$propertyId-${DateTime.now().millisecondsSinceEpoch}-$i';
