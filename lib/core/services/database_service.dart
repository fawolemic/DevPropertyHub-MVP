...
class DatabaseService {
  ...
  Future<Map<String, dynamic>> getDatabaseStats() async {
    try {
      ...
      final developmentCount =
          await _client.from('developments').select('*').count();
      final propertyCount =
          await _client.from('properties').select('*').count();
      final leadActivityCount =
          await _client.from('lead_activities').select('*').count();
  ...
  Future<Map<String, dynamic>?> safeInsert(
  ...
  Future<Map<String, dynamic>?> safeUpdate(
  ...
  Future<List<Map<String, dynamic>>> secureQuery({
    ...
    try {
      ...
      if (offset != null) {
        transformedQuery =
            transformedQuery.range(offset, offset + (limit ?? 10) - 1);
      }
