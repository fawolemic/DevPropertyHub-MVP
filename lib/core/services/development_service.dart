...
class DevelopmentService {
  ...
  Future<DevelopmentModel> createDevelopment(
      DevelopmentModel development) async {
    try {
  ...
  Future<List<DevelopmentModel>> getDevelopmentsByDeveloperId(
    ...
    try {
      ...
      return response
          .map((development) => DevelopmentModel.fromMap(development))
          .toList();
    } catch (e) {
  ...
  Future<DevelopmentModel> updateDevelopment(
      DevelopmentModel development) async {
    try {
      final developmentData = development.toMap();
  ...
  Future<void> deleteDevelopment(String developmentId) async {
    try {
      await _client.from('developments').delete().eq('id', developmentId);
    } catch (e) {
  ...
  Future<List<DevelopmentModel>> getDevelopmentsByStatus(
      String developerId, DevelopmentStatus status) async {
    try {
      final response = await _client
          .from('developments')
          .select()
  ...
  Future<List<DevelopmentModel>> searchDevelopments(
  ...
  Future<List<String>> uploadDevelopmentImages(
      String developmentId, List<dynamic> images) async {
    try {
      ...
      for (int i = 0; i < images.length; i++) {
        final file = images[i];
        final fileName =
            '$developmentId-${DateTime.now().millisecondsSinceEpoch}-$i';
        ...
        final imageUrl =
            _client.storage.from('development_images').getPublicUrl(response);
