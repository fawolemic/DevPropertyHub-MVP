...
class ApiServiceExampleScreen extends StatefulWidget {
  ...
  @override
  State<ApiServiceExampleScreen> createState() => _ApiServiceExampleScreenState();
}
  ...
  Future<void> _fetchUserData() async {
    ...
    try {
      ...
      final result = await _apiService.get(
        ...
        queryParams: {'include_preferences': 'true'}, // Optional query parameters
      );
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildStatusRow('Logged in:', authProvider.isLoggedIn.toString()),
                    _buildStatusRow('User type:', authProvider.userType.toString().split('.').last),
                    _buildStatusRow('User role:', authProvider.userRole.toString().split('.').last),
                    _buildStatusRow('Token expiry:', authProvider.isTokenExpired ? 'Expired' : 'Valid'),
                    if (authProvider.isDeveloper)
                      _buildStatusRow('Verification:', authProvider.verificationStatus ?? 'N/A'),
                    const Divider(),
                    Text(
