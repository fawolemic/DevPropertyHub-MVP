...
void main() {
  ...
  group('RBAC Service Tests', () {
    ...
    test('hasPermission uses caching correctly', () async {
      ...
      final secondResult =
          await rbacService.hasPermission('properties', 'read');
      expect(secondResult, true);
      verifyNever(mockAuthProvider.hasPermission(
          'properties', 'read')); // Should not be called again
    });
    ...
    test('canUpdate enforces explicit deny rules for buyers on properties',
        () async {
      final testUser = UserModel(
        id: 'test-user-id',
        email: 'test@example.com',
    ...
    test('canUpdate allows developers to update their own resources', () async {
      ...
      when(mockAuthProvider.isResourceOwner('properties', 'prop-123'))
          .thenAnswer((_) async => true);
      ...
      verify(mockAuthProvider.isResourceOwner('properties', 'prop-123'))
          .called(1);
    });
    ...
    test('isResourceOwner delegates to auth provider', () async {
      ...
      final result =
          await rbacService.isResourceOwner('properties', 'prop-123');
      expect(result, true);
