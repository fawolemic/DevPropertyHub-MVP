...
void main() {
  ...
  group('DatabaseService Tests', () {
    test('getDatabaseStats returns correct counts', () async {
      ...
      when(mockUserCount.count()).thenAnswer((_) async => PostgrestCountResponse(count: 10, data: []));
      
      when(mockClient.from('leads')).thenReturn(MockPostgrestBuilder());
      when(mockClient.from('leads').select('*')).thenReturn(mockLeadCount);
      when(mockLeadCount.count()).thenAnswer((_) async => PostgrestCountResponse(count: 5, data: []));
      
      when(mockClient.from('developments')).thenReturn(MockPostgrestBuilder());
      when(mockClient.from('developments').select('*')).thenReturn(mockDevelopmentCount);
      when(mockDevelopmentCount.count()).thenAnswer((_) async => PostgrestCountResponse(count: 3, data: []));
      
      when(mockClient.from('properties')).thenReturn(MockPostgrestBuilder());
      when(mockClient.from('properties').select('*')).thenReturn(mockPropertyCount);
      when(mockPropertyCount.count()).thenAnswer((_) async => PostgrestCountResponse(count: 15, data: []));
      
      when(mockClient.from('lead_activities')).thenReturn(MockPostgrestBuilder());
      when(mockClient.from('lead_activities').select('*')).thenReturn(mockLeadActivityCount);
      when(mockLeadActivityCount.count()).thenAnswer((_) async => PostgrestCountResponse(count: 8, data: []));
      ...
      expect(await databaseService.getDatabaseStats(), isA<Map<String, dynamic>>());
    });
    ...
    test('safeInsert handles data correctly', () async {
      ...
      when(mockFilterBuilder.single()).thenAnswer((_) async => {'id': 'new-id', 'email': 'test@example.com'});
    ...
    test('safeUpdate sets updated_at and handles data correctly', () async {
      ...
      // For now, we'll just test the method signature and error handling
      expect(await databaseService.safeUpdate('users', 'user-123', testData), isNull);
    });
    ...
    test('secureQuery builds query correctly', () async {
      ...
      when(mockFilterBuilder.eq('developer_id', 'dev-123')).thenReturn(mockFilterBuilder);
      ...
      // For now, we'll just test the method signature and error handling
      expect(await databaseService.secureQuery(
        table: 'properties',
        column: 'developer_id',
        value: 'dev-123',
        orderBy: 'created_at',
